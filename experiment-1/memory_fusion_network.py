...
def customLSTM(dim, name=None):
    ...
    if name is None:
        ...
        lstm = RNN(cell, return_sequences=True, name=name)
    return lstm
...
def MFN_unimodal(input_shapes, output_classes, mem_size=256):
    ...
    hc0 = customLSTM(dim0, name='modality0')(input0)
    # Split the above states from the concatenated result from LSTM
    h = Lambda(lambda x: x[:,:,:dim0], name='hidden_states')(hc0)
    
    c_prev = Lambda(lambda x: x[:,:,dim0:2*dim0])(hc0)
    c_new = Lambda(lambda x: x[:,:,2*dim0:], name='cell_states')(hc0)
    ...
    attended = Multiply(name='attended')([cStar, attention])
    cell = MultiviewGatedMemory(mem_size)
    lstm_cell = RNN(cell, name='memory')
    ...
    final = Concatenate(axis=-1, name='mfn_output')([last_hs, mem]) # Output of MFN
...
def MFN_bimodal(input_shapes, output_classes, mem_size=256):
    ...
    hc1 = customLSTM(dim1, name='modality1')(input1)
    ...
    c_new = Concatenate(axis=-1, name='cell_states')([c_new0, c_new1])
    cStar = Concatenate(axis=-1)([c_prev, c_new])
    ...
    h = Concatenate(axis=-1)([h0, h1], name='hidden_states')
    last_hs = Lambda(lambda x: x[:,-1,:])(h)
...
def MFN_trimodal(input_shapes, output_classes, mem_size=256):
    ...
    # Same procedure is followed for each modality
    hc2 = customLSTM(dim2, name='modality2')(input2)
    ...
    c_new = Concatenate(axis=-1, name='cell_states')([c_new0, c_new1, c_new2])
    cStar = Concatenate(axis=-1)([c_prev, c_new])
    ...
    h = Concatenate(axis=-1, name='hidden_states')([h0, h1, h2])
    last_hs = Lambda(lambda x: x[:,-1,:])(h)

    # this is treated as the final output given by MFN
    # to be used for classification
    final = Concatenate(axis=-1, name='mfn_output')([last_hs, mem])
